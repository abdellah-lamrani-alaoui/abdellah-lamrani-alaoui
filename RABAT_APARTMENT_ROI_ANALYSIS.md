# Rabat Apartment Investment Analysis — Best ROI in Low-Risk Neighborhoods

*Research date: June 30, 2026. Prices are indicative market ranges (DH/m²) compiled from current listings/market reports — always verify against live listings and get a notary/avocat to check title (titre foncier) before committing.*

## TL;DR Recommendation

**Buy a small studio or T1 (25-40 m²) in Agdal**, ideally near a tramway stop, targeting students/young professionals or furnished short-term rental. This is the configuration where Agdal's real yield edge actually shows up in matched, like-for-like listing data: **~5-8%**, vs. 4-6% in the most prestigious districts.

**Important update from deeper, size-matched comp analysis (Section 6)**: if you're buying a standard family-size 2-3BR apartment instead of a small unit, Agdal's yield advantage over Hay Riad mostly disappears — both land around **4.2-4.7%** on a like-for-like 80-90 m² 2BR. The yield premium is concentrated in small units, not family-size ones. So unit size is now part of the core recommendation, not just a nice-to-have.

If you want more upside and can tolerate slightly more risk, **Aviation** is the #2 pick: still undervalued, benefiting from tramway extension, good resale/appreciation potential, decent safety — matched comps put it around 5-6.5%.

Avoid chasing the highest headline yields (Yacoub El Mansour, Akkari, Salé Tabriquet/Bettana ~6.3-7.5%) — those come with weaker tenant profiles, lower liquidity, and more volatile resale value. Real comps reinforce this: rents for similar-sized Yacoub El Mansour units ranged 1,000-9,000 DH/month with no clear pattern — the kind of unpredictability that is exactly the risk you said you want to avoid.

---

## 1. Neighborhood comparison

| Neighborhood | Price range (DH/m²) | Gross rental yield | Safety / Prestige | Risk profile | Notes |
|---|---|---|---|---|---|
| **Agdal** | 12,000 – 18,000 | **5 – 8%** (best on studios/T2) | High — embassies, businesses, established | **Low** | Best yield/safety trade-off. Strong demand from students, young professionals, executives. 15-20% cheaper than Hay Riad with similar quality of life. |
| **Aviation** | 13,000 – 18,000 | ~6 – 7%+ (estimated, undervalued) | Medium-High, improving | **Low-Medium** | Still undervalued, benefiting from tramway connectivity and adjacent business zone growth. Good for buy-and-hold appreciation + furnished rental. |
| **Hay Riad** | 16,000 – 25,000 | 4 – 6% | Very High — diplomatic/business district | **Very Low** | Best capital preservation and price stability, but highest entry price suppresses yield. Good for F3 rented to expats/executives (10,000+ DH/month) but lower % return. |
| **Madinat Al Irfane** | ~10,000 – 14,000 (est.) | ~5 – 6.5% | High — near university & ministries | **Low** | Civil servant/family demand, steady but less dynamic than Agdal. Good for long-term tenant stability. |
| **Hassan / Océan** | 15,000 – 20,000 | Variable (5-7%+ if short-term/touristic) | High — central, historic, touristic | **Low-Medium** | Best for furnished/short-term (Airbnb) rental given tourist appeal; regulatory & seasonal vacancy risk for STR. |
| **Souissi** | 26,000 – 42,000 (mostly villas) | 3 – 5% | Very High — most exclusive | **Low (capital), poor yield** | Ultra-premium, family villas. Great for capital preservation, poor ROI as a yield play. |
| **Yacoub El Mansour** | 8,500 – 12,000 (~12,900 avg) | 6.3 – 7.5% | Medium | **Medium** | Higher nominal yield but weaker tenant profile/liquidity and slower appreciation. |
| **Akkari / Takkadoum** | 8,500 – 12,000 | 6.3 – 7.5% | Medium | **Medium** | Same trade-off as Yacoub El Mansour — affordable but more exposure if neighborhood doesn't gentrify as planned. |
| **Salé (Tabriquet/Bettana)** | Lower than Rabat proper | 6.3 – 7.5%+ | Medium-Low | **Medium-High** | Highest headline yields but across the river — weaker brand, longer-term price risk, harder resale to upscale buyers. |

Market-wide context: average gross rental yield for the Rabat-Salé area is around **6.7%**, with rent-to-price ratios of 0.46-0.56%/month. Vacancy for standard apartments runs 4-7%.

## 2. Why Agdal wins on your criteria ("highest ROI, still safe")

- **Yield**: small, well-located studios (25-30 m²) rent for 3,000-4,500 DH/month → 5-7% gross. Furnished T2s rent 6,000-7,500 DH/month → similarly strong yields, sometimes up to 8% on the smallest, most efficiently-bought units.
- **Demand depth**: students (nearby universities), young professionals, and executives all compete for Agdal units — this keeps vacancy low and supports rent growth, unlike riskier areas that depend on a single tenant segment.
- **Price discount vs. peers**: 15-20% cheaper per m² than Hay Riad or Souissi for comparable safety and services, which is what pushes the yield % higher without taking on real neighborhood risk.
- **Appreciation**: consistently cited as one of the fastest-appreciating central districts (alongside Hay Riad and Hassan), so you're not trading yield for stagnant resale value.

## 3. Practical buying strategy

1. **Unit type**: prioritize a small, efficient studio or T1/T2 (25-45 m²) over a large apartment — smaller units in Agdal post the best DH/m² yield because rent doesn't scale linearly down with size.
2. **Location within Agdal**: Bas Agdal (lower Agdal) is more commercial/affordable and rents well to students/young workers; Haut Agdal is calmer/pricier and suits longer-term family or executive tenants — pick based on whether you want max yield (Bas Agdal) or max stability (Haut Agdal).
3. **Proximity to tramway**: stay within a 5-10 min walk of a tram stop — this is repeatedly cited as the single biggest driver of both yield and liquidity at resale.
4. **Furnished vs. unfurnished**: furnished rentals to young professionals/expats command meaningfully higher rent in Agdal and Hassan; factor in furnishing cost (~50,000-100,000 DH) against the yield uplift.
5. **Financing/holding period**: a 5-7 year horizon is typically cited as needed to capture both rental income and capital appreciation — don't plan to flip in under 2 years given transaction costs (notary fees, registration ~6-7% of price).
6. **Due diligence**: confirm titre foncier (freehold title) is clean, check syndic/co-ownership charges, and verify the building isn't in an unresolved inheritance (indivision) situation — a common source of deal risk in older Agdal buildings.

## 4. If you want to diversify risk/return

- **Conservative core (lowest risk)**: Hay Riad apartment, rented to an expat/executive — lower yield (~5%) but best capital protection and easiest resale.
- **Core ROI pick (this analysis's top choice)**: Agdal studio/T2 — 5-8% yield, low risk.
- **Satellite / growth bet (higher upside, still acceptable risk)**: Aviation — undervalued today, tramway-driven appreciation likely over 5-7 years.
- **Avoid as a primary holding** unless you specifically want higher-risk/higher-yield exposure and can actively manage tenants: Yacoub El Mansour, Akkari, Salé.

---

## 5. Real listing-based yield validation (June 2026)

To sanity-check the market-report yields above, real for-sale and for-rent listings were pulled from Mubawab and Avito and matched by comparable size to compute actual gross yields.

| Neighborhood | Real sale comps (DH/m²) | Matched rent comps | Computed gross yield | Est. net yield* |
|---|---|---|---|---|
| **Agdal** | 15,766 – 22,667 (avg ~18,000) | Studio 35-40m² 3,100-4,500 DH/mo; 100m² furnished 13,000 DH/mo; 130m² 15,000 DH/mo | **6 – 8%** (up to ~9% on furnished small units) | ~4.5-6% |
| **Aviation** | 14,063 – 21,429 (small units pricier/m²) | 110-120m² 8,000 DH/mo; 64m² scaled ~4,300 DH/mo | **5 – 6.5%** | ~3.5-5% |
| **Hay Riad** | 20,476 – 27,350 (Prestigia developments) | 100-150m² Prestigia 9,500-12,000 DH/mo | **4.2 – 4.7%** | ~2.7-3.2% |
| **Yacoub El Mansour** | 15,385 – 20,000 (small, noisy sample) | Wide/inconsistent (3,400-22,000 DH/mo across 120-190m²) | ~6 – 7% (low confidence) | ~4.5-5.5% |

*Net = gross minus ~1.5-2.5 points for taxe d'habitation/professionnelle, syndic fees, ~5% vacancy, and maintenance.

**Conclusion**: real listings confirm Agdal is the standout — it beats Aviation on yield (not just on safety/liquidity), particularly for furnished mid-size units rented to professionals or small unfurnished units rented to students. Hay Riad's real yield is confirmed at the low end (4.2-4.7%) — you pay for safety and prestige, not income.

**Caveats**: these are asking prices/rents from live listings, not closing transactions (Moroccan sale prices typically negotiate 5-15% below asking, which would push real yields slightly higher). Sample sizes per neighborhood are small (search-based, a few listings each) — directional, not statistically robust. Before making an offer, pull 20-30 live comps directly from Mubawab/Avito filtered to your target unit size.

## 6. Strict size/type-matched comp analysis (refines Section 5)

Section 5 mixed unit sizes and furnished/unfurnished status, which inflates some yield estimates. This section re-does the comparison holding **size and type roughly constant** (~75-95 m², 2-bedroom, unfurnished, elevator+garage buildings) to get a fairer apples-to-apples read.

| Neighborhood | Sale comps used (DH/m²) | Rent comp (~80 m², unfurnished) | Computed gross yield | Confidence |
|---|---|---|---|---|
| **Agdal** | 8 comps: 15,766 / 17,500 / 17,836 / 18,537 / 19,643 / 19,663 / 22,667 / 23,171 (avg ~19,100) | 5,500-6,500 DH/mo | **4.2-4.7%** | High — tight, consistent comps |
| **Hay Riad** | 3 comps: 20,476 / 22,569 / 27,350 (avg ~23,500) | 7,600-8,930 DH/mo (scaled from Prestigia 100-150m² comps) | **4.2-4.7%** | High — tight comps, mostly one development (Prestigia) |
| **Aviation** | 5-6 comps: 12,500 / 12,774 / 14,063 / 15,455 / 16,170 (avg ~14,200) | 7,333-8,000 DH/mo | **~5-6.5%** credible range (one pair implied 13.7% — discarded as a likely condition mismatch) | Medium |
| **Yacoub El Mansour** | 4 clean comps: 7,000 / 8,500 / 10,000 / 10,000 (avg ~8,875). A 5th comp at 17,500 DH/m² was discarded — it was an exact duplicate of an unrelated Agdal listing, a data-contamination artifact from search-snippet aggregation | Highly inconsistent: 1,000-9,000 DH/mo across near-identical 70-80 m² units | Nominally 6.7-13.7%, but **not trustworthy** | Low — rent side too noisy to pin down |

### Key findings that update the recommendation

1. **Agdal's yield edge over Hay Riad mostly comes from small units, not family-size ones.** On a strict 80-90 m² 2BR comparison, Agdal and Hay Riad converge to nearly the same gross yield (~4.2-4.7%). The higher 6-8% figures in Section 5 partly came from comparing a furnished rental against an unfurnished sale price — not a fair match. Studio/T1 comps (23-40 m²) showed rent at 88-100+ DH/m²/month vs. ~69-75 DH/m²/month for family-size units, which is where the real premium lives.
2. **Yacoub El Mansour's headline yield looks more like data noise than a real edge.** Sale prices were consistent (~7,000-10,000 DH/m²), but rents for visually similar-sized units ranged 9x (1,000-9,000 DH/month) with no resolvable pattern from text descriptions alone. That unpredictability is itself evidence of the higher risk already flagged in Section 1.
3. **A clear data-contamination case (duplicate listing surfacing under two different neighborhoods) is a reminder that search-snippet research has real limits** — see the methodology note below.

### Methodology limitation: no visual/photo verification was possible

An attempt was made to inspect actual listing photos to verify unit condition/standing (the original intent behind "closely match by looking through images"), using two approaches:
- A headless browser (Playwright/Chromium) to navigate Avito/Mubawab/Yakeey directly — blocked by this environment's network policy (`403 policy denial` at the proxy level for these domains).
- The WebFetch tool on individual listing pages — blocked by the sites' own anti-bot protection (`403 Forbidden` from Mubawab, Avito, and Yakeey alike).

As a result, all matching in this report is **text-based** (size, room count, floor, stated condition keywords like "rénové"/"bon état"/"à rénover"), not photo-verified. The Aviation 13.7%-yield outlier and the Yacoub El Mansour rent dispersion above are exactly the kind of mismatch that photo verification would normally catch and exclude. Treat all yield figures as directional; verify unit condition in person or via an in-country agent before committing capital.

### Sources
- [Carte des prix de l'immobilier à Rabat — Yakeey](https://yakeey.com/fr-ma/referentiel-de-prix-immobilier/rabat)
- [Référentiel des prix immobiliers à Rabat — Agenz](https://agenz.ma/en/prix-immobilier-maroc/rabat)
- [Guide Immobilier Rabat 2025 — Sefiani Khadamat](https://sefiani-khadamat.com/guide-immobilier-quartiers-prises-rabat-2025/)
- [Guide Ultime 2025: Les Quartiers Premium de Rabat — aykana.ma](https://aykana.ma/en/guide-ultime-2025-les-quartiers-premium-les-plus-prises-de-rabat-analyse-prix-et-opportunites/)
- [Investir à Rabat en 2026 — Cyril Jarnias](https://www.cyriljarnias.com/immobilier-international/investir-immobilier-maroc/marche-immobilier-rabat-2026-analyses-previsions-investir/)
- [Investir dans l'immobilier à Rabat : Marché, Rendements et Meilleurs Quartiers — Cyril Jarnias](https://www.cyriljarnias.com/immobilier-international/investir-immobilier-maroc/investir-immobilier-rabat-marche-rendements-quartiers/)
- [Les meilleurs quartiers pour investir à Rabat en 2026 — Goterra](https://www.goterra.ai/fr/mobile/news/2e38ab3f-77a7-42cf-af78-84b74654947d)
- [Opportunités d'investissement locatif à Rabat — assurance-pret-immobiliers.com](https://www.assurance-pret-immobiliers.com/opportunites-d-investissement-locatif-a-rabat-analyse-du-marche/)
- [Guide Complet du Quartier Hay Riad à Rabat — Sefiani Khadamat](https://sefiani-khadamat.com/guide-complet-du-quartier-hay-riad-a-rabat-vivre-et-investir-dans-le-prestigieux-quartier-diplomatique/)
- [Vente Appartement Agdal Rabat — aykana.ma](https://aykana.ma/en/vente-appartement-agdal-rabat-le-guide-ultime-pour-investir-intelligemment-en-2025/)
- [Le Marché Immobilier à Rabat : prix par quartier — saleimmobilier.com](https://saleimmobilier.com/le-marche-immobilier-a-rabat/)
